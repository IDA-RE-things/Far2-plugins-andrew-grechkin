...
std::string w2cp(PCWSTR in, UINT cp) {
	CHAR buf[MAX_PATH];
	convert_cp(in, cp, buf, sizeof(buf));
	return std::string(buf);
}

std::wstring cp2w(PCSTR in, UINT cp) {
	WCHAR buf[MAX_PATH];
	convert_cp(in, cp, buf, sizeof(buf) / sizeof(buf[0]));
...
namespace Base
{
	...
	bool FrameInfo::Data::LoadFromPDB(size_t frame)
	{
		...
		SYMBOL_INFOW * m_syminfo;
		size_t size = sizeof(SYMBOL_INFOW) + 1024 * sizeof(WCHAR);
		m_syminfo = (SYMBOL_INFOW*)malloc(size);
		m_syminfo->SizeOfStruct = sizeof(*m_syminfo);
		m_syminfo->MaxNameLen = 1024;

		DWORD64 displacement;
		if (SymFromAddrW(GetCurrentProcess(), frame, &displacement, m_syminfo) != FALSE) {
...

		{
			...
			if (SymGetLineFromAddr64(GetCurrentProcess(), frame, &dummy, &info)) {
				...
				source = cp2w(info.FileName, CP_ACP);
			}
			printf("err: %d\n", GetLastError());
		}
	...
	ustring FrameInfo::AsStr() const
	{
		WCHAR buf[MAX_PATH];
		if (line())
			_snwprintf(buf, sizeof(buf) / sizeof(buf[0]), L"[%s] (%p) %s:0x%Ix {%s:%Iu}", module().c_str(), addr(), func().c_str(), offset(), source().c_str(), line());
		else
			_snwprintf(buf, sizeof(buf) / sizeof(buf[0]), L"[%s] (%p) %s:0x%Ix", module().c_str(), addr(), func().c_str(), offset());
		return ustring(buf);
	}
	...
	Backtrace::Backtrace(size_t depth)
	{
		...
		while (depth-- > 0) {
			...
			if (!res || sf.AddrReturn.Offset == 0)
				break;
			emplace_back((size_t)sf.AddrReturn.Offset);
#else
