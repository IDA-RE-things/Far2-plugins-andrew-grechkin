...
Privilege::~Privilege() {
	if (m_disable) {
		...
		if (hToken.IsOK()) {
			...
			::AdjustTokenPrivileges(hToken, false, &m_tp, sizeof(m_tp), nullptr, nullptr);
		}
	}
}

Privilege::Privilege(PCWSTR name): m_disable(false) {
	WinToken hToken(TOKEN_QUERY | TOKEN_ADJUST_PRIVILEGES);
	if (hToken.IsOK()) {
		LUID luid;
		if (::LookupPrivilegeValueW(nullptr, name, &luid)) {
			...
			if (::PrivilegeCheck(hToken, &ps, &Result) && !Result) {
				m_tp.PrivilegeCount = 1;
				m_tp.Privileges[0].Luid = luid;
				m_tp.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;
				m_disable = ::AdjustTokenPrivileges(hToken, false, &m_tp, sizeof(m_tp), nullptr, nullptr);
			}
		}
	}
}
