...
void SaveSettings() {
	...
	if (reg.OpenKey(_T("Console"), KEY_WRITE)) {
		WCHAR ColorName[] = _T("ColorTable00");
		for (uint i = 0x10; i--;) {
			ColorName[10] = i / 10 + '0';
			ColorName[11] = i % 10 + '0';
...
//------------------------------------------------------------------------
	...
	WCHAR inFont[MAX_PATH], inFont2[MAX_PATH];
...
BOOL CALLBACK wndOpProc(HWND hWnd2, UINT messg, WPARAM wParam, LPARAM lParam) {
	switch (messg) {
		case WM_INITDIALOG:
			...
			if (gSet.isRClickSendKey)CheckDlgButton(hWnd2, cbRClick, BST_CHECKED);
			if (gSet.isCursorV)
...
bool LoadImageFrom(WCHAR *inPath) {
	WCHAR exPath[MAX_PATH + 2];
