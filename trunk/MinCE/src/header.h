...
#define MBox(rt) (int)MessageBox(nullptr, rt, Title, MB_SYSTEMMODAL | MB_ICONINFORMATION)
#define MBoxA(rt) (int)MessageBoxA(nullptr, rt, "Information", MB_SYSTEMMODAL | MB_ICONINFORMATION)
...
public:
	...
	template <class T> void Save(const TCHAR *regKey, T value) {
		RegSetValueEx(regMy, regKey, nullptr, REG_BINARY, (LPBYTE)(&value), sizeof(T));
	}
	void Save(const TCHAR *regKey, const TCHAR *value) {
		RegSetValueEx(regMy, regKey, nullptr, REG_SZ, (LPBYTE)(value), _tcslen(value) * sizeof(TCHAR));
	}
	...
	template <class T> bool Load(const TCHAR *regKey, T *value) {
		DWORD len = sizeof(T);
		if (RegQueryValueEx(regMy, regKey, nullptr, nullptr, (LPBYTE)(value), &len) == ERROR_SUCCESS)
			return true;
		return false;
	}
...
struct VirtualConsole {
	...
	VirtualConsole(HANDLE hConsoleOutput = nullptr);
	~VirtualConsole();
