...
namespace Ext {
	...
	void set_security(PCWSTR path, PSECURITY_DESCRIPTOR sd, SE_OBJECT_TYPE type) {
		DWORD flag = 0;
...
//		}
		...
		DWORD err = ::SetNamedSecurityInfoW((PWSTR)path, type, flag, owner, group, dacl, sacl);
		if (err == ERROR_INVALID_OWNER) {
			WinPriv::modify(SE_TAKE_OWNERSHIP_NAME, true);
			WinPriv::modify(SE_RESTORE_NAME, true);
			err = ::SetNamedSecurityInfoW((PWSTR)path, type, flag, owner, group, dacl, sacl);
		}
		CheckApiError(err);
	}
