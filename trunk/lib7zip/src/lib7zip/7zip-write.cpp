...
namespace SevenZip {
	DirItem::DirItem(const ustring & file_path, const ustring & file_name):
		WinFileInfo(Base::MakePath(file_path, file_name).c_str()),
		path(file_path),
		name(file_name) {
	}
	...
	void CompressProperties::add(const ustring & add_path) {
		...
		path = Base::get_fullpath(Base::ensure_no_end_path_separator(path));
		path = Base::ensure_path_prefix(path);
		if (Ext::FS::is_exist(path)) {
	...
	void CompressProperties::printf(PCWSTR format, ...) const {
		va_list vl;
		va_start(vl, format);
		Base::stdvprintf(STD_OUTPUT_HANDLE, format, vl);
		va_end(vl);
	}
	...
	void CompressProperties::base_add(const ustring & base_path, const ustring & name) {
		...
		if (Ext::FS::is_dir(path)) {
			Ext::WinDir dir(path);
			for (Ext::WinDir::iterator it = dir.begin(); it != dir.end(); ++it) {
