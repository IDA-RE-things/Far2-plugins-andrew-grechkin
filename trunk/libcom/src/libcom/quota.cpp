...
void QuotaInfo::set_limit(size_t in) {
	CheckApiError(m_usr->SetQuotaLimit(Base::mega_to_bytes(in), true));
}

void QuotaInfo::set_threshold(size_t in) {
	CheckApiError(m_usr->SetQuotaThreshold(Base::mega_to_bytes(in), true));
}
...
void DiskQuota::set_default_limit(size_t in) const {
	CheckApiError(m_control->SetDefaultQuotaLimit(Base::mega_to_bytes(in)));
}

void DiskQuota::set_default_threshold(size_t in) const {
	CheckApiError(m_control->SetDefaultQuotaThreshold(Base::mega_to_bytes(in)));
}
...
size_t DiskQuota::get_default_limit() const {
	...
	return Base::bytes_to_mega(Result);
}
...
ComObject<IDiskQuotaUser> DiskQuota::add_raw(const ustring & name, size_t lim, size_t thr) {
	ComObject<IDiskQuotaUser> ret;
	CheckApiError(m_control->AddUserName(name.c_str(), DISKQUOTA_USERNAME_RESOLVE_ASYNC, &ret));
	CheckApiError(ret->SetQuotaLimit(Base::mega_to_bytes(lim), true));
	CheckApiError(ret->SetQuotaThreshold(Base::mega_to_bytes(thr), true));
	return ret;
}
