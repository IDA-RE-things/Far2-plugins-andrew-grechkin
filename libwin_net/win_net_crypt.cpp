...
bool			Base64::Decode(PCSTR in, WinBuf<BYTE> &buf, DWORD flags) {
	DWORD	size = 0;
	if (::CryptStringToBinaryA(in, 0, flags, NULL, &size, NULL, NULL)) {
		buf.reserve(size);
		return	::CryptStringToBinaryA(in, 0, flags, buf, &size, NULL, NULL);
	}
	return	false;
}
bool			Base64::Decode(PCWSTR in, WinBuf<BYTE> &buf, DWORD flags) {
	DWORD	size = 0;
	if (::CryptStringToBinaryW(in, 0, flags, NULL, &size, NULL, NULL)) {
		buf.reserve(size);
		return	::CryptStringToBinaryW(in, 0, flags, buf, &size, NULL, NULL);
	}
	return	false;
}
CStrA			Base64::EncodeA(PVOID buf, DWORD size, DWORD flags) {
	CStrA	Result;
	DWORD	len = 0;
	if (::CryptBinaryToStringA((const PBYTE)buf, size, flags, NULL, &len)) {
		Result.reserve(len);
...
AutoUTF			Base64::Encode(PVOID buf, DWORD size, DWORD flags) {
	AutoUTF	Result;
	DWORD	len = 0;
	if (::CryptBinaryToStringW((const PBYTE)buf, size, flags, NULL, &len)) {
		Result.reserve(len);
...
//		DWORD	dwStrType = CERT_X500_NAME_STR;
		...
		if (ChkSucc(::CertStrToNameW(enc, in.c_str(), dwStrType, NULL, m_info.pbData, &m_info.cbData, NULL))) {
			WinMem::Alloc(m_info.pbData, m_info.cbData);
			ChkSucc(::CertStrToNameW(enc, in.c_str(), dwStrType, NULL, m_info.pbData, &m_info.cbData, NULL));
		}
	}
...
///========================================================================================= WinCert
WinCert::WinCert(PCCERT_CONTEXT in): m_cert(NULL) {
	if (in)
		m_cert = ::CertDuplicateCertificateContext(in);
}
WinCert::WinCert(const WinCert &in): m_cert(NULL) {
	if (in.m_cert)
...
//				ext.rgExtension = rrr;
				...
				m_cert = ::CertCreateSelfSignCertificate(provider, &blob, 0, &info, NULL, NULL, until, NULL);
				if (m_cert) {
//					CTL_USAGE	usage;
...
bool				WinCert::ToFile(const AutoUTF &path) const {
	...
	if (m_cert) {
		...
		if (store.IsOK()) {
			if (::CertAddCertificateContextToStore(store, m_cert, CERT_STORE_ADD_NEW, NULL)) {
...
bool				WinCert::Store(HANDLE in) {
	return	ChkSucc(::CertAddCertificateContextToStore(in, m_cert, CERT_STORE_ADD_ALWAYS, NULL));
}
AutoUTF				WinCert::GetAttr(DWORD in) const {
	size_t	size = ::CertGetNameStringW(m_cert, in, 0, NULL, NULL, 0);
	WCHAR	buf[size];
	if (::CertGetNameStringW(m_cert, in, 0, NULL, buf, size)) {
		return	buf;
	}
	return	AutoUTF();
}
AutoUTF				WinCert::GetProp(DWORD in) const {
	AutoUTF	Result;
	DWORD	cbData = 0;
	::CertGetCertificateContextProperty(m_cert, in, NULL, &cbData);
	if (cbData) {
		WCHAR	buf[cbData];
...
CStrA				WinCert::GetHashString() const {
	CStrA	Result;
	DWORD	cbData = 0;
	::CertGetCertificateContextProperty(m_cert, CERT_HASH_PROP_ID, NULL, &cbData);
	if (cbData) {
...
size_t				WinCert::GetHashSize() const {
	DWORD	Result = 0;
	::CertGetCertificateContextProperty(m_cert, CERT_HASH_PROP_ID, NULL, &Result);
	return	Result;
}
...
AutoUTF				WinCert::GetProp(PCCERT_CONTEXT pctx, DWORD in) {
	AutoUTF	Result;
	DWORD	cbData = 0;
	::CertGetCertificateContextProperty(pctx, in, NULL, &cbData);
	if (cbData) {
...
CStrA				WinCert::HashString(PCCERT_CONTEXT pctx) {
	CStrA	Result;
	DWORD	cbData = 0;
	::CertGetCertificateContextProperty(pctx, CERT_HASH_PROP_ID, NULL, &cbData);
	if (cbData) {
		WinBuf<BYTE>	buf(cbData);
