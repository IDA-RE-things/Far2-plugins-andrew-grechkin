...
void	PassSave(PCWSTR name, PCWSTR pass) {
	...
	cred.CredentialBlob = (PBYTE)prot;
	cred.CredentialBlobSize = sizeof(*prot) * (Len(prot) + 1);
	CheckAPI(CredWrite(&cred, 0));
}
...
void	PassList() {
	/*
		...
		for (size_t i = 0; i < cnt; ++i) {
			cout << "cre [" << i << "] flags: " << cre[i]->Flags;
			cout << " Name: " << cre[i]->TargetName;
			cout << " Blob: " << (PCWSTR)cre[i]->CredentialBlob << endl;
		}
	*/
}
