...
///========================================================================================== Base64
namespace	Base64 {
	Base::auto_array<BYTE> Decode(PCSTR in, DWORD flags = CRYPT_STRING_BASE64_ANY);

	Base::auto_array<BYTE> Decode(PCWSTR in, DWORD flags = CRYPT_STRING_BASE64_ANY);

	astring EncodeA(const Base::auto_array<BYTE> & buf, DWORD flags = CRYPT_STRING_BASE64);

	ustring	Encode(const Base::auto_array<BYTE> & buf, DWORD flags = CRYPT_STRING_BASE64);
...
namespace Crypt {
	...
	struct Provider: private Base::Uncopyable {
		~Provider();
	...
	///============================================================================ CertificateStore
	struct CertificateStore: private Base::Uncopyable {
		enum StoreType {
			stMachine,
			stUser,
			stMemory,
	...
	struct Certificate {
		...
		Base::auto_array<BYTE> get_hash() const;
